...
<template>
    <div
        ...
        <div
            v-if="!inputStore.forceInput && editMode"
            class="my-2 md:my-4 md:grid md:grid-rows-2 md:grid-cols-2 flex flex-row gap-1 w-24 pointer-events-auto">
            <Btn
        ...
        <div
            id="inputComp"
            class="flex md:flex-col max-w-full overflow-auto flex-row justify-stretch align-stretch content-stretch pointer-events-auto">
            <InputComp/>
        </div>
        <!-- Repeated 3 times for contrast -->
        <div v-if="editMode" position="relative w-full">
            <p
                class="absolute md:text-center md:w-full text-sm font-normal drop-shadow-md pointer-events-none select-none mix-blend-saturation whitespace-pre-line">
                {{ getNbBriqs }}
            </p>
            <p
                class="absolute md:text-center md:w-full text-sm font-normal drop-shadow-md pointer-events-none select-none mix-blend-difference whitespace-pre-line">
                {{ getNbBriqs }}
            </p>
            <Tooltip tooltip="This gives you the # of briqs used in the set, out of the total available briqs.">
                <p
                    class="absolute md:text-center md:w-full text-sm font-normal drop-shadow-md pointer-events-auto select-none mix-blend-overlay whitespace-pre-line">
                    {{ getNbBriqs }}
                </p>
            </Tooltip>
        </div>
