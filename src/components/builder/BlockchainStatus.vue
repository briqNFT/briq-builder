...
<template>
    <div class="flex flex-col items-end">
        ...
        <div class="overflow-auto flex flex-nowrap flex-col justify-start content-end">
            <Btn
                class="pointer-events-auto flex items-center gap-2 text-center"
                @click="mainBtnClick"
                :tooltip="mainBtnTooltip">
                <div v-if="!isConnected">Connect wallet</div>
                <div v-else-if="isConnected && contractStore.briq" class="inline-block text-sm leading-3">
                    {{ userAddress }}<br>{{ userNet }}
                </div>
                <div v-else="" class="inline-block text-sm leading-3">Unsupported<br>network</div>
                <template v-if="isConnected && contractStore.briq">
                    <i
                        v-if="transactionsManager.anyPending() || chainBriqs.fetchingBriqs"
                        class="fas fa-spinner animate-spin-slow"/>
                    <i
                        v-if="
                            !transactionsManager.anyPending() &&
                                !chainBriqs.fetchingBriqs &&
                                chainBriqs.getNbBriqs() > 0
                        "
                        class="fas fa-check"/>
                </template>
            </Btn>
        ...
        <div :class="'relative bg-grad-lightest rounded ' + (expanded ? 'pointer-events-auto' : 'hidden')">
            <BriqDetails/>
        </div>
        <div :class="'my-2 relative bg-grad-lightest rounded ' + (expanded ? 'pointer-events-auto' : 'hidden')">
            <TransactionsMin/>
        </div>
    </div>
</template>
