...
<template>
    <div>
        ...
        <Btn v-bind="$attrs" @click="builderHistory.undo()" :disabled="!canUndo" class="w-10 h-10" tooltip="Undo (Ctrl + Z)">
            <p><i class="fas fa-undo"/></p>
        </Btn>
        <Btn v-bind="$attrs" @click="builderHistory.redo()" :disabled="!canRedo" class="w-10 h-10" tooltip="Redo (Ctrl + Shift + Z)">
