...
<template>
    ...
    <div class="flex-1 overflow-auto">
        <div
            :class="
                'grid grid-rows-2 grid-flow-col gap-0.5 ' +
                    (palette.getNbColors() < 20 ? ' md:flex md:flex-col' : ' md:grid-cols-3 md:grid-flow-row')
            ">
            <div
            ...
            <div class="flex flex-col" v-for="(value, key) in palette.colors" :key="key">
                <Btn
                    class="h-5 min-h-0 shadow-md"
                    :tooltip="'Select color ' + value"
                    @click="pickColor(key)"
                    :style="{
                        backgroundColor: key,
                        border:
                            inputStore.currentInput != 'place_nft' && currentColor === key
                                ? '3px solid black'
                                : '0px solid black',
                    }"/>
            </div>
