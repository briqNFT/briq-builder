...
<template>
    <div class="my-4">
        ...
        <p class="flex gap-2 my-4">
            <Btn tooltip="Create a new WIP set." @click="createSet"><i class="far fa-file"/> New</Btn>
            <Btn tooltip="Import a local set or a .vox file" @click="importSet">
                <i class="fas fa-file-import"/> Import from file
            </Btn>
            <Btn tooltip="Delete all selected sets." @click="deleteAll" :disabled="!canDelete">
                <i class="far fa-trash-alt"/> Delete local sets
            </Btn>
            <Btn v-if="canResetMigrationPrompt" tooltip="Un-hide all hidden legacy sets." @click="resetMigrationPrompt">
                <i class="fas fa-eye"/> Show hidden legacy sets
            </Btn>
            <!--
    ...
    <div class="grid w-full grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        ...
        <SetGridItem
            v-for="setId in chainSets"
            :key="setId"
            :set-id="setId"
            :search-text="searchText"
            :open-details="openDetails === setId"
        ...
        <div
            ...
            <Tooltip tooltip="Ignore this set and hide the migration prompt.">
                <button @click="hideMigrationPrompt(oldSet)" class="absolute top-0 right-0 px-4 py-4 font-semibold">
