                ...
                <div class="flex flex-nowrap relative" :style="{ left: `-${Math.min(step('SIGNING'), step(exporting))*100}%` }">
                    <div class="flex-none w-full">
                        ...
                        <div class="text-lg font-semibold">
                            <p v-if="transactionPending">An export is already ongoing - see Transaction {{ pending_transaction?.hash }}</p>
                    ...
                    <div class="flex-none w-full">
                        ...
                        <div class="flex justify-around my-8">
                            ...
                            <div class="flex flex-col justify-start basis-1/2 text-center">
                                <p><button class="block mx-auto btn" :disabled="transactionPending || !hasBriqsAndSets || notEnoughBriqs"
                                    @click="exportSetOnChain">Migrate on chain</button></p>
                    ...
                    <div :class="'flex-none w-full'">
                        <div class="text-lg font-medium">
                            ...
                            <Btn class="my-2" v-if="exporting === 'ERROR'" @click="exporting = 'CONFIRMATION'; errorStep = undefined">Start over</Btn>
                        </div>
                    </div>
                </div>
            </div>
...
type ExportSteps = 'PRECHECKS' | 'CONFIRMATION' | 'SIGNING' | 'SENDING_TRANSACTION' | 'WAITING_FOR_CONFIRMATION' | 'DISASSEMBLING' | 'ERROR' | 'DONE';
const exportSteps = ['PRECHECKS', 'CONFIRMATION', 'SIGNING', 'SENDING_TRANSACTION', 'WAITING_FOR_CONFIRMATION', 'DISASSEMBLING', 'ERROR', 'DONE'];
