...
<template>
    <div v-if="notifs.length" class="text-sm flex flex-col-reverse">
        <div v-for="notif, i of notifs" :key="i" class="w-full">
            <div :class="['w-full text-left relative flex justify-between rounded-sm p-2', notif.read ? 'text-darkest' : 'hover:bg-darker'].join(' ')" @click="notif.read = true">
                <div class="grow">
                    <p>{{ notif.summary }}</p>
                    <p class="text-xs text-darkest">
                        {{ notif.timestamp }}
                ...
                <p class="basis-[20px] text-right text-accent text-lg">{{ notif.read ? '' : '•' }}</p>
            </div>
            <div v-if="details === i" class="bg-gray-100 p-2 border-t border-darker rounded-b-md w-full">
                <component :is="notif.render()" :notif="notif"/>
            </div>
        </div>
    </div>
    <div v-else>
        <p class="px-4 py-2 text-darkest select-none text-sm">You don't have any notifications</p>
    </div>
</template>
