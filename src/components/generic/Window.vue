    ...
    <div ref="window" :class="'container p-0 rounded-md md:rounded-lg bg-grad-lightest alternate-buttons m-2 sm:m-4 md:m-8 shadow-xl relative ' + _size">
        <div class="relative h-full p-4 md:p-6">
            <div class="flex justify-between items-center pb-6">
                <h3 class="font-semibold w-full flex items-center" v-if="!!$slots.title"><slot name="title"/></h3>
                <h2 class="font-semibold w-full flex items-center" v-if="!!$slots['big-title']"><slot name="big-title"/></h2>

                <button
                    @click="close"
                    class="text-xl inline-flex justify-center items-center h-10 w-10 p-0">
